...
import com.enterprise.erp.reports.application.dto.SalesDailyTotalProjection;
...
public interface SalesOrderJpaRepository extends JpaRepository<SalesOrderJpaEntity, Long> {
    ...
    @EntityGraph(attributePaths = {"client", "user", "warehouse", "lines", "lines.product"})
    @Query("SELECT o FROM SalesOrderJpaEntity o WHERE o.id = :id")
    Optional<SalesOrderJpaEntity> findDetailedById(@Param("id") Long id);

    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT o FROM SalesOrderJpaEntity o WHERE o.id = :id")
    Optional<SalesOrderJpaEntity> findByIdForUpdate(@Param("id") Long id);
